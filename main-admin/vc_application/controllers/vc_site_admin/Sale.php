...
class Sale extends CI_Controller {
  ...
  if($data['customer_info'][0]['parent_customer_id']!='') { $data['sponser_name'] = $this->sale_model->get_customer_info($data['customer_info'][0]['parent_customer_id']);  }
   ...
   public function gst_report() {
    ...
    public function generatecsv(){
	 ...
	 if ($this->input->post('sdate') != '' && $this->input->post('edate') != '') {
        ...
        // $payouts = $this->sale_model->get_all_total_order_items($sdate,$edate);
        // echo '<pre>'; print_r($payouts); echo '</pre>'; die;
        // die;
   ...
   $file = fopen('php://output', 'w');
    ...
    $header = array("Date and Time","Inv No","Name of Party","Nature","GSTN","Product","HSN","Rate","Taxable","CGST","SCGST","IGST","Tax Rate","Tax Amount","Inv Total Val","Place of shipping"); 
    fputcsv($file, $header);
   	$payouts = $this->sale_model->get_all_total_order_items($sdate,$edate);
   	if(!empty($payouts)) {
		    $i = 1;
       foreach ($payouts as $key=>$line){
        $tax_ratio = (100+$line['tax'])/100;
        $actual_price = $line['price']/$tax_ratio;
        $tax_amount = $line['price'] - $actual_price;

        if(strtolower($line['p_state']) == 'karnataka') {
          $cgst = $line['tax']/2;
          $scgst = $line['tax']/2;
          $igst = 0;
        } else {
	     ...
	     $csv_val = array(date('d F Y',strtotime($line['rdate'])),$line['invoice'],$line['f_name'].' '.$line['l_name'].'','','',$line['product_title'],$line['hsn'],$line['price'],$actual_price,$cgst,$scgst,$igst,$line['tax'],$tax_amount,$line['price'],$line['p_state']);
