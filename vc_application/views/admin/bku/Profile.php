...
private $right_user = array();
  ...
  public function index() {
	...
	if($url=='admin') { redirect(base_url('distributor'));  }
		...
		$data['repurchase_bv'] = $this->Users_model->fetch_table_data('repurchase_bv',$id);
		$data['directs'] = $this->Users_model->get_direct_user_ids($customer_id);
	/*	$customers = array();
		$get_all = $this->Users_model->get_all();
		if(!empty($get_all)) {
		    foreach($get_all as $cus) {
		        $directs = $this->Users_model->get_all_dir($cus['customer_id']);
		        $dire = COUNT($directs);
		        $customers[] = array('id'=>$cus['id'],'direct'=>$dire);
		    }
		}
		echo '<pre>'; echo '</pre>';
		$this->Users_model->update_batch('customer',$customers,'id'); */
		$data['bliss_perk_history'] = ''; //$this->Users_model->bliss_perk_history($id);
		//$pv_info = $this->Users_model->get_pv($cid);

		$data['redeem_error'] = '';
		$binary_income = $this->Users_model->fetch_table_data_sum('binary_income',$id);
		$binary_level_income = $this->Users_model->fetch_table_data_sum('binary_level_income',$id);
		$direct_income = $this->Users_model->fetch_table_data_sum('direct_income',$id);
		$turnover_income = $this->Users_model->fetch_table_data_sum('turnover_income',$id);
		$roality_income = $this->Users_model->fetch_table_data_sum('roality_income',$id);
		$data['total_income'] = $binary_income[0]['amount']+$binary_level_income[0]['amount']+$direct_income[0]['amount']+$turnover_income[0]['amount']+$roality_income[0]['amount'];
		$data['invite_email'] = '';
		if ($this->input->server('REQUEST_METHOD') && $this->input->post('submit4')=='Share via Email') {
			$to = $this->input->post('email');
            $subject = "invitation from rtionline.co.in";
            $txt = $this->input->post('textmsg');
            $headers = "From: rtionline.co.in" . "\r\n";
			
			$headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= 'From: rtionline <info@rtionline.co.in>' . "\r\n";
  ...
  public function get_friend_by_id($customer_id){
   ...
   public function add_pv_income($cid,$caping,$user_level,$reward,$position,$distribution_amount,$cust_id,$order_id,$repurchase,$lbv,$rbv,$count_left,$count_right,$left_child,$right_child) { 
		...
		if(!empty($pv_info)) { 
			...
			$matching_pv = $this->Users_model->get_matching_pv($cid);
				...
				$repurchase = '';
				  ...
				  if($customer_id != '' && $cust_id != '' && $distribution_amount > 0 && $pin[0]['franchisee'] == '0'  ) { 
					...
					if($user[0]['user_level']=='0') {
						...
						$package = array('user_level'=>$user_level,'package'=>$user_level,'capping'=>$pin[0]['capping'],'package_used'=>$date,'consume'=>1,'plan_name'=>$pin[0]['p_amount'],'sbv'=>$distribution_amount);
						...
						/*************** direct sponcer ****************/	
						$add_income = array('user_id'=>$user[0]['did'],'payby'=>$cust_id,'amount'=>$pin[0]['b_volume']*(10/100),'net_income'=>$pin[0]['p_amount']*(5/100));
				...
				$position = $user[0]['position']; 
					...
					while($p < 1) {
						...
						if(!empty($parent_user)) {
						  ...
						  if($position == 'left') {
							if($user[0]['user_level']=='0') { $plcount = $parent_user[0]['plcount']+1;  } else { $plcount = $parent_user[0]['plcount']; }
							 $count_bv[] = array('id'=>$parent_user[0]['id'],'lbv'=>$parent_user[0]['lbv']+$distribution_amount,'rbv'=>$parent_user[0]['rbv'],'plcount'=>$plcount,'prcount'=>$parent_user[0]['prcount'],'left_sale'=>$parent_user[0]['left_sale']+$p_amount,'right_sale'=>$parent_user[0]['right_sale']);
						  }
						 else {
							 if($user[0]['user_level']=='0') { $prcount = $parent_user[0]['prcount']+1;  } else { $prcount = $parent_user[0]['prcount']; }
							 $count_bv[] = array('id'=>$parent_user[0]['id'],'lbv'=>$parent_user[0]['lbv'],'rbv'=>$parent_user[0]['rbv']+$distribution_amount,'plcount'=>$parent_user[0]['plcount'],'prcount'=>$prcount,'left_sale'=>$parent_user[0]['left_sale'],'right_sale'=>$parent_user[0]['right_sale']+$p_amount); 
						 }
				  ...
				  if($distribution == TRUE ) {
				      ...
				      if(!empty($this->matching_amount)) {
						...
						$dir_id = array(); 
						$direct_id = $this->Users_model->get_first_two_direct_user_ids($parent_user[0]['direct_customer_id']);
						if(!empty($direct_id)) {
