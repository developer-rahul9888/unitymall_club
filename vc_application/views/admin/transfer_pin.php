      ...
      <?php echo form_close(); 
		 ...
		 <fieldset>
		   <div class="form-group col-sm-4">
			...
			<p><label>PV: </label>&nbsp;<?php echo $pin[0]['b_volume'];?></p>
			<!--p><label>Transactional PIN: </label> <input type="text" value="" required name="tpin"></p-->
			<input type="hidden" value="<?php echo $pin[0]['p_amount'];?>" required name="pin_amt">
			<?php  if($pin[0]['franchisee'] == 0) { ?>
  ...
  <option selected disabled>Select Product</option>
		  ...
		  foreach($product as $products){
            echo"<option product=".$products['p_d_price']." value=".$products['id']."~~".$products['p_d_price']."~~".$products['bv']."~~".str_replace(' ', '-', $products['pname'])."~~".$products['t_class'].">".$products['pname']."   Amount:- ".$products['p_d_price']. " ( PV :- ".$products['bv']. ")</option>";
		    }	
		  }
          ...
          <div class="form-group  col-lg-12">
            <button class="btn btn-success" type="submit">Activate PIN</button> &nbsp; 
			 <a class="btn btn-primary" href="<?php echo base_url().'admin/pin_activate/'.$this->uri->segment(3); ?>">Back </a>
          </div>
        </fieldset>  
